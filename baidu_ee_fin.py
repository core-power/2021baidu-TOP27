...
# 基本信息
train=False  #训练的时候设置为True 预测的时候设置为Fals.
maxlen = 400
epochs = 10
batch_size = 4
learning_rate = 1e-5
crf_lr_multiplier = 100  # 必要时扩大CRF层的学习率
model_save='best_model.weights'
# bert配置
        ...
        for l in f:
            ...
            for event in l['event_list']:
                for argument in event['arguments']:
                    ...
                    arguments[key] = value
            D.append((l['text']+'||'+l['title'], arguments))
    return D


# 读取数据
train_data = load_data('../baidu_ee_fin/duee_fin_train_pro.json')
valid_data = load_data('../baidu_ee_fin/duee_fin_dev.json')
add_data = load_data('../baidu_ee_fin/duee_fin_dev_pro.json')
# 读取schema
with open('../baidu_ee_fin/duee_fin_event_schema.json') as f:
...
def predict_to_file(in_file, out_file):
    ...
    with open(in_file) as fr:
        ...
        for l in tqdm(fr):
            ...
            if len(list(l['text']))>400:
                text_list = (l['text']).split('\n')
                event = []
                event_list = []
                for t in text_list:
                    arguments = extract_arguments(t+'||'+l['title'])
                    #try:
                         #l['event_list']
                    #except:continue

                    for k, v in arguments.items():
                        event_list.append({'event_type': v[0], 'arguments': [{'role': v[1], 'argument': k}]})
                event_list = list_dict_duplicate_removal(event_list)
                l['event_list'] = event_list
                
                event_type = []
                for i in event_list:
                    event_type.append(i['event_type'])
                event_type = list(set(event_type))
                for i in event_type:
                    dict = {}
                    for j in event_list:
                        if j['event_type'] == i:
                            dict['event_type'] = i
                            dict['arguments'] = dict.get('arguments', []) + j['arguments']
                    event.append(dict)
                pred_json={'id':l['id'],'event_list':event}
                pred_json = json.dumps(pred_json, ensure_ascii=False)
                fw.write(pred_json + '\n')
            else:
                    arguments = extract_arguments(l['text']+'||'+l['title'])
                    event_list = []
                    for k, v in arguments.items():
                        event_list.append({'event_type': v[0], 'arguments': [{'role': v[1], 'argument': k}]})
                    event_list = list_dict_duplicate_removal(event_list)
...
if __name__ == '__main__':
    if train==True:
        ...
        #dev_generator =  data_generator(add_data, batch_size)
        evaluator = Evaluator()
        ...
        model.load_weights(model_save)
        predict_to_file('/home/maxin/baidu_data/baidu_ee_fin/duee_fin_test1.json', 'duee-fin.json')
